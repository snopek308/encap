 ...
 * that control what data is allowed through validation -- Code that is repeated
 * (Don't Repeat Yourself or DRY) is encapsulated in a helper method called
 * "getFormattedDate". Such methods don't have to be private but this one is
 * because it has no use elsewhere. -- Methods that need to be called in a
 * specific order are encapsulated in a single parent method called
 * "doFirstTimeOrientation". This assures that the order will be followed. But
 * even if order where not a priority this code would still benefit from the use
 * of parent method/helper method architecture. If forces each method routine to
 ...
 * themselves. -- Not all methods are public. Having too many public methods
 ...
 * classes. -- Notice that a separate EmployeeReportService object is created so
 ...
 * Also, this is a reusable object that can be used elsewhere. -- Other classes
 * representing nouns in the problem domain are used to make the simulation
